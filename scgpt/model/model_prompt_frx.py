...
# from .transformer_flash_attn import FlashTransformerEncoderLayer_Local
...
class TransformerModel(nn.Module):
    def __init__(
        ...
        self.rank = rank
        self.rank_encoder_type = rank_encoder_type
        ...
        # Rank Encoder
        if self.rank:
            # self.rank_encoder = RankEncoder(max_len, d_model, padding_idx=rank_pad_value)
            if rank_encoder_type == 'Emb':
                self.rank_encoder = RankEncoder(max_len, d_model, padding_idx=rank_pad_value)
            elif rank_encoder_type == 'SinCos':
                self.rank_encoder = RankEncoder_PositionalEmbedding(d_model=d_model, max_len=max_len - 1)
            elif rank_encoder_type == 'MLP':
                self.rank_encoder = ContinuousValueEncoder(d_model, dropout)
            elif rank_encoder_type == 'Leanable':
                self.rank_encoder = nn.Parameter(torch.zeros(1, max_len - 1, d_model))
        ...
        if use_fast_transformer:
            if fast_transformer_backend == "linear":
                ...
                pass
                # self.transformer_encoder = FlashTransformerEncoderLayer_Local(
                #     nlayers=nlayers,
                #     norm=None,
    ...
    def _encode(
        ...
        if self.rank:
            if self.rank_encoder_type == "Leanable":
                rank_values = self.rank_encoder
            else:
                rank_values = self.rank_encoder(ranks)
        ...
        if self.rank:
            total_embs_expect_cls = total_embs.clone()[:, 1:, :]
            cls_embs = total_embs.clone()[:, 0:1, :]
            total_embs_with_rank = total_embs_expect_cls + rank_values
            total_embs = torch.cat((cls_embs, total_embs_with_rank), dim=1)
    ...
    def forward(
        ...
        transformer_output = self._encode(
            src, ranks, values, src_key_padding_mask, batch_labels
        )
        if self.use_batch_labels:
            batch_emb = self.batch_encoder(batch_labels)  # (batch, embsize)
...
class TransformerModel_NoRank(nn.Module):
    def __init__(
            self,
            ntoken: int,
            d_model: int,
        ...
        if do_deconv:
            self.spot_encoder = nn.Embedding(3, d_model, padding_idx=2)

        # projection
        self.do_proj = do_proj
        if do_proj:
            self.projection = Projection(embedding_dim=d_model, projection_dim=d_model)
        ...
        self.do_deconvdecoder = do_deconvdecoder
        if do_deconvdecoder:
            self.deconv_decoder = DeconvDecoder(
                d_model,
                celltype_number=deconv_celltype_number
            )
    ...
    def _encode(
        ...
        if self.do_deconv:
            spot = self.spot_encoder(input_spot_flags)
            total_embs += spot
    ...
    def encode_multiple_target(self, cell_args: dict, spot_args: dict):
        # for cells
        CLS = cell_args["CLS"]
        cell_args.pop("CLS")

        transformer_output = self._encode(**cell_args)
        output = {}
        ...
        # for spot
        transformer_output = self._encode(**spot_args)
        spot_emb = self._get_cell_emb_from_layer(transformer_output)
        spot_emb = self.projection(spot_emb)
        output["spot_proj_emb"] = spot_emb
    ...
    def decoder_deconv(self,
        ...
        mlm_output = self.deconv_decoder(cell_emb)

        deconv_output = mlm_output["pred"]  # (batch, seq_len)

        return deconv_output

    def _encode_kaiming_collapsing(
            self,
            src: Tensor,
            values: Tensor,
            src_key_padding_mask: Tensor,
        ...
        # self.projection is projection MLP Simsiam: https://arxiv.org/abs/2011.10566
        output = self.projection(output)
    ...
    def forward(
        ...
        if self.use_prompt and self.prompt_type == 'encoder-prompt':
            ...
            if self.norm_scheme == "pre":
                ...
                if self.space_adapter_conf[index] == 1:
                    src2 = self.Space_Adapter(src2)
                src = src + self.dropout1(src2)
                src = self.norm2(src)
                src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
                src = src + self.dropout2(src2)
                if self.mlp_adapter_conf[index] == 1:
                    src = src + self.scale * self.MLP_Adapter(src)
            else:
                src2 = self.self_attn(src, key_padding_mask=src_key_padding_mask_)[0]
                if self.space_adapter_conf[index] == 1:
                    src2 = self.Space_Adapter(src2)
...
class GeneEncoder(nn.Module):
    def __init__(
        ...
        if self.use_prompt and self.prompt_type == 'head-prompt':
            self.Space_Adapter = Adapter(embedding_dim)

        if self.use_prompt and self.prompt_type == 'condition-prompt':
            self.prompt_embeddings = nn.ParameterList(
                [nn.Parameter(torch.zeros(1, 1, embedding_dim)) for i in range(self.num_tokens)])
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.use_prompt and self.prompt_type == 'condition-prompt':
            ...
            for prompt_emb in self.prompt_embeddings:
                prompt_emb = torch.repeat_interleave(prompt_emb, repeats=x.shape[0], dim=0)
                cls = torch.add(cls, prompt_emb)
            x = torch.cat((cls, x[:, 1:, :]), dim=1)

        return x


class RankEncoder_PositionalEmbedding(nn.Module):
...
class DeconvDecoder(nn.Module):
    def __init__(
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(d_model, d_model),
            nn.LeakyReLU(),
            nn.Linear(d_model, celltype_number),
        )
    ...
    def forward(self, x: Tensor) -> Dict[str, Tensor]:
        """x is the output of the transformer, (batch, seq_len, d_model)"""
        pred_value = self.fc(x).squeeze(-1)  # (batch, seq_len)
        # pred_value = self.active_Relu(pred_value)
        return dict(pred=pred_value)
