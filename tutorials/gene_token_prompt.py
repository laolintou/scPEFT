...
hyperparameter_defaults = dict(
    ...
    use_prompt=True,
    prompt_type='head-prompt',  # encoder-prompt、prefix-prompt、head-prompt、condition-prompt、finetune、LoRA
    num_tokens=64,
...
save_dir = Path(f"./save/{prompt_type}/scGPT-{dataset_name}-{prompt_type}/")
...
if dataset_name == "ms":
    ...
    # adata = sc.read(data_dir / f"ms_train_{exp_name}.h5ad")
    adata = sc.read(data_dir / f"ms_train.h5ad")
    adata_val = sc.read(data_dir / "ms_val.h5ad")
    ...
    adata = sc.read(data_dir / "Mye_train.h5ad")
    adata_val = sc.read(data_dir / "Mye_val.h5ad")
    adata_test = sc.read(data_dir / "Mye_test.h5ad")
...
elif dataset_name == "hPancreas":
    adata = sc.read(data_dir / "hPancreas_train.h5ad")
    adata_val = sc.read(data_dir / "hPancreas_val.h5ad")
    adata_test = sc.read(data_dir / "hPancreas_test.h5ad")
    adata.obs["celltype"] = adata.obs["cell_type"].astype("category")
    adata_test.obs["celltype"] = adata_test.obs["cell_type"].astype("category")
    ...
    adata = sc.read(data_dir / "mouse_1215_train.h5ad")
    adata_val = sc.read(data_dir / "mouse_1215_val.h5ad")
    adata_test = sc.read(data_dir / "mouse_1215_test.h5ad")
    adata.obs["celltype"] = adata.obs["celltype"].astype("category")
    ...
    adata_test_raw = adata_test.copy()
    adata = adata.concatenate((adata_test, adata_val), batch_key="str_batch")
elif dataset_name == "Mouse_CD8TIL_Tony":
    adata = sc.read(data_dir / "Mouse_CD8TIL_Tony_train.h5ad")
    adata_val = sc.read(data_dir / "Mouse_CD8TIL_Tony_val.h5ad")
    adata_test = sc.read(data_dir / "Mouse_CD8TIL_Tony_test.h5ad")
    adata.obs["celltype"] = adata.obs["seurat_clusters"].astype("category")
    adata_val.obs["celltype"] = adata_val.obs["seurat_clusters"].astype("category")
    adata_test.obs["celltype"] = adata_test.obs["seurat_clusters"].astype("category")
...
# pd.DataFrame.from_dict(celltype_dict, orient='index', columns=['celltype']).to_csv(
#     f'./{dataset_name}_{prompt_type}_{exp_name}_celltype_code.csv')
...
if config.use_prompt and config.prompt_type == 'LoRA':
